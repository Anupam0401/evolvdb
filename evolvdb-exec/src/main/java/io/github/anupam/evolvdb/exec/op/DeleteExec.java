...
/**
 * Executes DELETE statements by pulling matching tuples from the child operator and deleting them.
 * Uses the Volcano pull model: the child (e.g. FilterExec wrapping SeqScanWithRidExec) provides
 * both the tuple and its RecordId. Collects all RecordIds first, then deletes, to avoid modifying
 * the heap file while iterating.
 */
public final class DeleteExec implements PhysicalOperator {
    ...
    public Tuple next() throws Exception {
        ...
        for (Tuple row = child.next(); row != null; row = child.next()) {
            ...
            if (rid == null) {
                throw new IllegalStateException("DELETE child operator does not provide RecordIds");
            }
            ridsToDelete.add(rid);
        }
