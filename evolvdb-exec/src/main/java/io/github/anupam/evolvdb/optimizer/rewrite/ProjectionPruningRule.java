...
/**
 * Inserts narrowing LogicalProject nodes to eliminate unused columns in subtrees. Conservative:
 * does not change existing Project schemas (avoids type inference).
 */
public final class ProjectionPruningRule implements LogicalRule {
    ...
    private Set<ColumnRef> requiredFromRoot(LogicalPlan root) {
        // If root is a project, only its expressions are required; else conservatively require all
        // root columns
    ...
    private LogicalPlan pruneRec(LogicalPlan node, Set<ColumnRef> requiredAbove) {
        if (node instanceof LogicalFilter f) {
            ...
            return (narrowed == child)
                    ? new LogicalFilter(child, f.predicate())
                    : new LogicalFilter(narrowed, f.predicate());
        }
        if (node instanceof LogicalProject p) {
            // Determine which outputs are required; compute child requirements from used
            // expressions
            ...
            if (kept.isEmpty()) {
                // Nothing from this project is required: produce a constant TRUE project with one
                // column to keep schema valid
            ...
            return new LogicalProject(
                    child, p.items(), p.schema()); // keep original schema to avoid type recompute
        }
        if (node instanceof LogicalAggregate a) {
            ...
            for (ProjectItem it : a.aggregates())
                here.addAll(ExprUtils.collectColumnRefs(it.expr()));
            here.addAll(requiredAbove); // downstream may expect outputs
            LogicalPlan child = pruneRec(a.child(), here);
            LogicalPlan narrowed = maybeProject(child, here);
            return (narrowed == child)
                    ? new LogicalAggregate(child, a.groupBy(), a.aggregates(), a.schema())
    ...
    private LogicalPlan maybeProject(LogicalPlan child, Set<ColumnRef> required) {
        ...
        for (ColumnMeta cm : keep) {
            ...
            items.add(
                    new ProjectItem(
                            new ColumnRef(new SourcePos(1, 1), null, cm.name()), cm.name()));
        }
    ...
    private static String normalizeName(ColumnRef c) {
        return c.table() == null
                ? c.column().toLowerCase(Locale.ROOT)
    ...
    private List<ColumnMeta> mapRequiredToColumns(Set<ColumnRef> required, Schema schema) {
        ...
        for (ColumnMeta cm : schema.columns()) {
            ...
            if (wantsFull.contains(name)
                    || wantsShort.contains(name)
                    || wantsFull.contains(shortName)
                    || wantsShort.contains(shortName)) {
                keep.add(cm);
            }
        }
        if (keep.isEmpty()) {
