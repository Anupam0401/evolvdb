...
public class ExprEvaluatorTest {
    ...
    void eval_arithmetic() {
        // Integer arithmetic
        Expr add =
                new BinaryExpr(pos, BinaryExpr.Op.ADD, new Literal(pos, 10), new Literal(pos, 5));
        assertEquals(15, evaluator.eval(add, null, null));

        Expr sub =
                new BinaryExpr(pos, BinaryExpr.Op.SUB, new Literal(pos, 10), new Literal(pos, 3));
        assertEquals(7, evaluator.eval(sub, null, null));

        Expr mul = new BinaryExpr(pos, BinaryExpr.Op.MUL, new Literal(pos, 4), new Literal(pos, 5));
        assertEquals(20, evaluator.eval(mul, null, null));

        Expr div =
                new BinaryExpr(pos, BinaryExpr.Op.DIV, new Literal(pos, 20), new Literal(pos, 4));
        assertEquals(5, evaluator.eval(div, null, null));

        // Float arithmetic
        Expr floatAdd =
                new BinaryExpr(
                        pos, BinaryExpr.Op.ADD, new Literal(pos, 3.5f), new Literal(pos, 2.5f));
        assertEquals(6.0f, evaluator.eval(floatAdd, null, null));

        // Mixed types (int + float = float)
        Expr mixed =
                new BinaryExpr(
                        pos, BinaryExpr.Op.ADD, new Literal(pos, 10), new Literal(pos, 2.5f));
    ...
    void eval_comparison() {
        ...
        Expr eq =
                new ComparisonExpr(
                        pos, ComparisonExpr.Op.EQ, new Literal(pos, 10), new Literal(pos, 10));
        assertEquals(true, evaluator.eval(eq, null, null));

        Expr neq =
                new ComparisonExpr(
                        pos, ComparisonExpr.Op.NEQ, new Literal(pos, 10), new Literal(pos, 5));
        assertEquals(true, evaluator.eval(neq, null, null));

        Expr lt =
                new ComparisonExpr(
                        pos, ComparisonExpr.Op.LT, new Literal(pos, 5), new Literal(pos, 10));
        assertEquals(true, evaluator.eval(lt, null, null));

        Expr lte =
                new ComparisonExpr(
                        pos, ComparisonExpr.Op.LTE, new Literal(pos, 10), new Literal(pos, 10));
        assertEquals(true, evaluator.eval(lte, null, null));

        Expr gt =
                new ComparisonExpr(
                        pos, ComparisonExpr.Op.GT, new Literal(pos, 15), new Literal(pos, 10));
        assertEquals(true, evaluator.eval(gt, null, null));

        Expr gte =
                new ComparisonExpr(
                        pos, ComparisonExpr.Op.GTE, new Literal(pos, 10), new Literal(pos, 10));
    ...
    void eval_logical() {
        ...
        Expr andTrue =
                new LogicalExpr(
                        pos, LogicalExpr.Op.AND, new Literal(pos, true), new Literal(pos, true));
        assertEquals(true, evaluator.eval(andTrue, null, null));

        Expr andFalse =
                new LogicalExpr(
                        pos, LogicalExpr.Op.AND, new Literal(pos, true), new Literal(pos, false));
        assertEquals(false, evaluator.eval(andFalse, null, null));

        // OR
        Expr orTrue =
                new LogicalExpr(
                        pos, LogicalExpr.Op.OR, new Literal(pos, true), new Literal(pos, false));
        assertEquals(true, evaluator.eval(orTrue, null, null));

        Expr orFalse =
                new LogicalExpr(
                        pos, LogicalExpr.Op.OR, new Literal(pos, false), new Literal(pos, false));
        assertEquals(false, evaluator.eval(orFalse, null, null));

        // NOT
        Expr notTrue = new LogicalExpr(pos, LogicalExpr.Op.NOT, new Literal(pos, true), null);
        assertEquals(false, evaluator.eval(notTrue, null, null));

        Expr notFalse = new LogicalExpr(pos, LogicalExpr.Op.NOT, new Literal(pos, false), null);
    ...
    void eval_complex_expression() {
        // (id + 10) * 2 > 50
        Schema schema = new Schema(List.of(new ColumnMeta("id", Type.INT, null)));
        Tuple tuple = new Tuple(schema, List.of(25));

        Expr complex =
                new ComparisonExpr(
                        pos,
                        ComparisonExpr.Op.GT,
                        new BinaryExpr(
                                pos,
                                BinaryExpr.Op.MUL,
                                new BinaryExpr(
