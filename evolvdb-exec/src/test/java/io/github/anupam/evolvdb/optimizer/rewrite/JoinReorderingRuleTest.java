    ...
    void greedy_left_deep_starts_with_smallest_table() {
        ...
        Expr jd1 =
                new ComparisonExpr(
                        ...
                        new ColumnRef(new SourcePos(1, 1), "f", "id"),
                        new ColumnRef(new SourcePos(1, 1), "d1", "fid"));
        Expr jd2 =
                new ComparisonExpr(
                        new SourcePos(1, 1),
                        ComparisonExpr.Op.EQ,
                        new ColumnRef(new SourcePos(1, 1), "f", "id"),
                        new ColumnRef(new SourcePos(1, 1), "d2", "fid"));

        // Build initial order: ((f ⋈ d2) ⋈ d1)
        LogicalJoin j1 =
                new LogicalJoin(fact, dim2, LogicalJoin.JoinType.INNER, jd2, fact.schema());
        LogicalJoin root =
                new LogicalJoin(j1, dim1, LogicalJoin.JoinType.INNER, jd1, fact.schema());

        InMemoryStatsProvider stats =
                new InMemoryStatsProvider()
        ...
        assertEquals(
                "dim1",
                leftmost.tableName(),
                "Smallest table should be left-most in left-deep tree");
    }
