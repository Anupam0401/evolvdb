        ...
        switch (col.type()) {
            case INT -> {
                if (!(v instanceof Integer))
                    throw new IllegalArgumentException(col.name() + " expects INT");
            }
            case BIGINT -> {
                if (!(v instanceof Long))
                    throw new IllegalArgumentException(col.name() + " expects BIGINT");
            }
            case BOOLEAN -> {
                if (!(v instanceof Boolean))
                    throw new IllegalArgumentException(col.name() + " expects BOOLEAN");
            }
            case FLOAT -> {
                if (!(v instanceof Float))
                    throw new IllegalArgumentException(col.name() + " expects FLOAT");
            }
            case STRING -> {
                if (!(v instanceof String))
                    throw new IllegalArgumentException(col.name() + " expects STRING");
            }
            case VARCHAR -> {
                if (!(v instanceof String s))
                    throw new IllegalArgumentException(col.name() + " expects VARCHAR");
                int max = Objects.requireNonNull(col.length(), "varchar length");
                if (s.length() > max)
                    throw new IllegalArgumentException(
                            col.name() + " exceeds VARCHAR(" + max + ")");
            }
