...
final class Tokenizer {
    ...
    private Token readString() {
        ...
        while (!eof()) {
            ...
            if (c == '\'') {
                ...
                // end string
                return new Token(
                        TokenType.STRING, sb.toString(), new SourcePos(startLine, startCol));
            }
            sb.append(c);
        }
    ...
    private char peekAhead(int off) {
        int i = index + off;
    ...
    private Token token(TokenType t, String lex) {
        return new Token(t, lex, new SourcePos(line, col - (lex == null ? 0 : lex.length())));
    }
    ...
    private static SqlParseException error(String msg, SourcePos pos) {
        return new SqlParseException(msg, pos);
    }
}
